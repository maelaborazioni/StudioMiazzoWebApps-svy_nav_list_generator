...
function addField(event, _type) {
	...
	else if (_type == 'footer')
	{
		...
		//Create variable
		var _variable = _jsForm.newVariable(_elementName,JSColumn.NUMBER)
...
function printPreview(event) {
	...
	//copy the variables
	var _variables = solutionModel.getForm(_formname).getVariables();//forms[_formname].allvariables
...
function saveList(event) {
	...
	if(globals.ltg_print_list_id) //a current list
	{
		...
		_answer = globals.svy_mod_dialogs_global_showQuestionDialog('svy.fr.lbl.excuse_me', _message, _save, _new)
		if(_answer == _save)
		{
			_newList = false
		}
	...
	_rec.program = globals.ltg_program_name
...
function loadExisting(event) {
	...
	{
		globals.svy_mod_dialogs_global_showInfoDialog('svy.fr.lbl.excuse_me','svy.fr.dlg.no_print_list','svy.fr.lbl.ok')
		return
	}
...
function loadPrintList(a, b, c, d, e, _print_list_id) {
	...
	var _rec = _to_nav_print_list.getSelectedRecord()
	globals.ltg_program_name = _rec.program
	...
	for (var i = 1; i <= _rec.nav_print_list_to_nav_print_list_details.getSize(); i++) {
		...
		else
		{
			...
			{
				//Create variable
				var _variable = _jsForm.newVariable(_rec_d.elementname,JSColumn.NUMBER)
...
function show(_program, _foundset) {
	...
	globals.svy_mod_showFormInDialog(forms.ltg_main)
}
